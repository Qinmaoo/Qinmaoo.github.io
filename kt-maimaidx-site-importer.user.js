...
function insertImportButton(message, onClick) {
	if (!getPreference(API_KEY) && window.confirm("You don't have an API key set up. Please set up an API key before proceeding.")) {
		location.href = "https://maimaidx.jp/maimai-mobile/home/"
	}
...
function getChartType(row) {
	...
	return chartTypeImg.src
		.replace("https://maimaidx.jp/maimai-mobile/img/music_", "")
		.replace(".png", "");
}

function getDifficulty(row, selector, style) {
	let difficulty = row.querySelector(selector).src
		.replace("https://maimaidx.jp/maimai-mobile/img/diff_", "").replace(".png", "")
...
async function isNiconicoLink(detailIdx = null) {
	const html = await fetch(`https://maimaidx.jp/maimai-mobile/record/musicDetail/?idx=${encodeURIComponent(detailIdx)}`).then(r => r.text())
	...
	for (let i = 0; i < scoresElems.length; i++) {
		...
		if (clearStatusElement !== null) {
			clearStatus = clearStatusElement.src
				.replace("https://maimaidx.jp/maimai-mobile/img/playlog/", "").replace(".png", "")
		}
		const lampStatus = e.querySelector(".playlog_result_innerblock.basic_block.p_5.f_13").children[1].src
			.replace("https://maimaidx.jp/maimai-mobile/img/playlog/", "").replace(".png?ver=1.35", "")
	...
	for (let i = 0; i < 5; i++) {
		...
		for (const e of elems) {
			...
			const lampElem = e.querySelectorAll(".h_30.f_r")[1].src
				.replace("https://maimaidx.jp/maimai-mobile/img/music_icon_", "")
...
async function executeDanAndClassImport() {
	...
	let danNumber = Number(danBadge.replace("https://maimaidx.jp/maimai-mobile/img/course/course_rank_", "")
	...
	const classNumber = Number(classBadge.replace("https://maimaidx.jp/maimai-mobile/img/class/class_rank_s_", "").replace(".png", "").substring(0, 2));
